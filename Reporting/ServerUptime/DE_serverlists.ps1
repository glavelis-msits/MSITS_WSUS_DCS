...
$latest_app_de_list_output  | Select-Object -Skip 1 | Out-File $latest_app_de_path\"clean_"$latest_app_de_list
...
$latest_dba_de_list_output  | Select-Object -Skip 1 | Out-File $latest_dba_de_path\"clean_"$latest_dba_de_list
...
$latest_trm_de_list_output  | Select-Object -Skip 1 | Out-File $latest_trm_de_path\"clean_"$latest_trm_de_list
...
do
 {
    ...
    {
    '1' {
    $de_appdc = Get-ADComputer -Filter 'dnshostname -like "*.mmsrg.net"' -SearchBase "OU=Domain Controllers,DC=mmsrg,DC=net" -Properties IPv4Address | FT DNSHostName -A -HideTableHeaders;
    $de_appdc | Out-File "$ScriptDir\DE_serverlists_reports\$(get-date -f dd-MM-yyyy)-APP_DC_ServerList.txt" -force ;
    ServerList-DE_APP
    } '2' {
    $de_dba = Get-ADComputer -Filter 'dnshostname -like "*.mmsrg.net"' -SearchBase "OU=DBA,OU=DE,OU=Server,DC=mmsrg,DC=net" -Properties IPv4Address | FT DNSHostName -A -HideTableHeaders;
    $de_dba | Out-File "$ScriptDir\DE_serverlists_reports\$(get-date -f dd-MM-yyyy)-DBA_ServerList.txt" -force;
    ServerList-DE_DBA
     } '3' {
    $de_trm = Get-ADComputer -Filter 'dnshostname -like "*.mmsrg.net"' -SearchBase "OU=TRM,OU=DE,OU=Server,DC=mmsrg,DC=net" -Properties IPv4Address | FT DNSHostName -A -HideTableHeaders;
