...
$sourcePath_wsus_local_update_noreboot = "$ScriptDir\DBA_DE_wsus_local_update_reboot.ps1"  	#Local WSUS update script path
$destPath_wsus_local_update_noreboot = "C:\tasks"                                          	#WSUS update path destination
$sourcePath_WSUS_Update_check_xml = "$ScriptDir\DBA_DE_Test_Group_WSUS_Monthly_Update.xml" 	#Scheduled Update Task local xml path
$destPath_WSUS_Update_check_xml = "C:\temp\wsus"                                           	#Scheduled Task Destination 
$sourcePath_DBA_DE_powercycle = "$ScriptDir\DBA_DE_reboot.ps1"   							#Local Powercycle script path
$destPath_DBA_DE_powercycle = "C:\tasks" 													#Powercycle script destination	
$sourcePath_DBA_DE_reboot_xml = "$ScriptDir\DBA_DE_reboot.xml" 								#Scheduled Reboot Task local xml path
$destPath_DBA_DE_reboot_xml = "C:\temp\wsus"     											#Reboot Task xml destination
#$servers = "$ScriptDir\FQDNList.txt"														#Server List
$servers = $latest_DBA_DE_list                                                              #Server List (new)
...
$latest_DBA_DE_path = "$ScriptDir\Reporting\ServerUptime\DE_serverlists_reports"            #Server List path
$latest_DBA_DE_list = (Get-ChildItem -Path $latest_DBA_DE_path -filter *clean*APP* | Sort-Object LastAccessTime -Descending | Select-Object -First 1).Name # Select the latest APP/DC list
...
$DBA_DE_wsus_upd = "C:\tasks\DBA_DE_wsus_local_update_reboot.ps1"
...
$DBA_DE_wsus_task_xml = "C:\temp\wsus\DBA_DE_Test_Group_WSUS_Monthly_Update.xml"
...
# Copy DBA_DE_reboot script	
$DBA_DE_reboot_path = "C:\tasks\DBA_DE_reboot.ps1"
if (Test-Path $DBA_DE_reboot_path -PathType leaf) 
{"Server reboot script exists" } 
else
{copy-item -Path $sourcePath_DBA_DE_powercycle -Destination $destPath_DBA_DE_powercycle -recurse -ToSession $Session -ErrorAction SilentlyContinue} ;
...
Get-Content $servers| ForEach-Object {
    #$Session = New-PSSession -ComputerName "$_" ;
	Invoke-Command -ComputerName "$_" -ScriptBlock {Set-ExecutionPolicy unrestricted -force; Register-ScheduledTask -xml (Get-Content 'C:\temp\wsus\DBA_DE_Test_Group_WSUS_Monthly_Update.xml' | Out-String) -TaskName "DBA_DE_Test_Group_WSUS_Monthly_Update" -TaskPath "\" -User mmsrg\SVC-TaskAutomateCopy  -Password isRIvx0Vbu5V61nEnq56 –Force}
    }

#Task DBA_DE_reboot
Get-Content $servers| ForEach-Object {
    #$Session = New-PSSession -ComputerName "$_" ;
	Invoke-Command -ComputerName "$_" -ScriptBlock {Set-ExecutionPolicy unrestricted -force ; Register-ScheduledTask -xml (Get-Content 'C:\temp\wsus\DBA_DE_reboot.xml' | Out-String) -TaskName "DBA_DE_reboot" -TaskPath "\" -User mmsrg\SVC-TaskAutomateCopy  -Password isRIvx0Vbu5V61nEnq56 –Force}
    } 