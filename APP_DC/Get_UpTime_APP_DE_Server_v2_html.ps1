...
$APP_wsupdrb_path = "\\$s\c$\tasks\APP_DE_wsus_local_update_reboot.ps1"    				# WSUS Update Script verification
$APP_wsupdrb = if (Test-Path $APP_wsupdrb_path -PathType leaf) 
{"Exists"}
else
{"Missing"}
...
$wsustaskrb = Invoke-Command -ComputerName $s {(Get-ScheduledTask | Where-Object {$_.TaskName -eq "APP_DE_reboot"}).State}
...
Import-CSV "$ScriptDir\temp_APP.csv" | ConvertTo-Html -Head $css  | Out-File "$ScriptDir\reporting\APPDC_DE_Report-$(get-date -f dd-MM-yyyy).html" 
rm "$ScriptDir\temp_APP.csv"
rm "$ScriptDir\APP_DE_ServerList_temp.txt"