...
#Get-UpTimeAllServer | Out-File "$ScriptDir\$(get-date -f dd-MM-yyyy)-TRM_DE_ServerInfo.log" -force
Get-UpTimeAllServer |  ConvertTo-Html | Out-File "$ScriptDir\$(get-date -f dd-MM-yyyy)-ServerUptime.html" -force
#Get-UpTimeAllServer | Export-Csv -Path "$ScriptDir\$(get-date -f dd-MM-yyyy)-ServerUptime.csv" -Encoding ascii -NoTypeInformation
